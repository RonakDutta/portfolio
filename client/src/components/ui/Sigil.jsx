...
/**
 * The mark: an "R" struck into a diamond of iron, rim still hot from the forge.
 *
 * One shape does three jobs, which is the whole point of having a mark at all.
 * It is the nav logo, it is the centrepiece of the threshold, and the same
 * geometry is cut into `public/favicon.svg` so the browser tab, the home-screen
 * icon and the header all show the same object.
 *
 * The letter is a path, not text. `font-family: Cinzel` here would mean the mark
 * renders in Times for the first few hundred milliseconds of every cold load,
 * which is exactly the window the threshold exists to control, and it would
 * break outright in the favicon, where no webfont is ever loaded.
 *
 * Gradient ids are scoped with useId. Two instances mount at once during the
 * threshold hand-off, and duplicate ids in the same document mean the second
 * sigil silently paints with the first one's fill.
 */

/** Cinzel Black "R", 24 units tall, pre-centred in the 64-unit box. */
const R_PATH =
...
/**
 * The diamond. Its perimeter is 167 units (four sides of 29.5√2), which is the
 * dash length the threshold's rim-draw in index.html is written against. Change
 * the shape and that number has to move with it.
 */
